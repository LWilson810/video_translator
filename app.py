...
# if video.startswith('http://') is False or video.startswith('https://') is False:
    ...
    os.system("move /Y audio2\\audio\\*.ogg audio\\")
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Group():
                    ...
                    min_speakers = gr.Slider(1, MAX_TTS, label="min_speakers", step=1, visible=False)
                ...
                with gr.Column():
                    with gr.Accordion("Advanced Settings", open=False):
                        ...
                        WHISPER_MODEL_SIZE = gr.Dropdown(['large-v1', 'large-v2'], value=whisper_model_default, label="Whisper model")
                        batch_size = gr.Slider(1, 32, label="Batch size", step=1)
                        compute_type = gr.Dropdown(list_compute_type, value=compute_type_default, label="Compute type")
...
### link
    ...
    #             blink_input = gr.Textbox(label="Video link", info="Example: www.youtube.com/watch?v=g_9rPvbENUw", placeholder="URL goes here...")
    ...
    #             # bline_ = gr.HTML("<hr style='margin-bottom:0'>")
    #             with gr.Group():
    #                 gr.Markdown(normal_setting)
    #                 # gr.Markdown("Select how many people are speaking in the video.")
    #                 bmin_speakers = gr.Slider(1, MAX_TTS, label="min_speakers", step=1, visible=False)
    #                 bmax_speakers = gr.Slider(1, MAX_TTS, value=2, step=1, label="Max speakers", interactive=True)
    #                 gr.Markdown("Select the voice you want for each speaker.")
    #                 def bsubmit(value):
    #                     visibility_dict = {
    ...
    #                     gr.HTML("<hr style='margin-bottom:0'>")
    #                     gr.Markdown("Default configuration of Whisper.")
    #                     bWHISPER_MODEL_SIZE = gr.Dropdown(['large-v1', 'large-v2'], value="large-v1", label="Whisper model")
    #                     bbatch_size = gr.Slider(1, 32, label="Batch size", step=1)
    #                     bcompute_type = gr.Dropdown(list_compute_type, value="float32", label="Compute type")

    #                     gr.HTML("<hr style='margin-bottom:0'>")
    #                     today1 = datetime.datetime.now().timestamp
    #                     output_filename1 = f"video_{today}.mp4"
    #                     bVIDEO_OUTPUT_NAME = gr.Textbox(label="Translated file name" ,value=output_filename1, info="The name of the output file")
    ...
    #                 blink_output = gr.Video(label="Preview")
